...
namespace CrytonCore.Helpers
{
    public class MD5Helper
    {
		...
		sealed public class Digest
		{
			...
			{
				string st;
				st = MDHelper.ReverseByte(A).ToString("X8") +
					MDHelper.ReverseByte(B).ToString("X8") +
					MDHelper.ReverseByte(C).ToString("X8") +
					MDHelper.ReverseByte(D).ToString("X8");
				return st;
		...
		{
			public readonly byte[] NewData;
			public readonly string FingerPrintResult;
